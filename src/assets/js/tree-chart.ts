...
class TreeChart {
    ...
    updateOrgUnits(
        ...
        orgUnitSelection
            ...
            .duration(this.duration)
            .attr('x', function (data) {
                if (data?.data?.name === 'Su Miao') {
                    console.log(data)
                    console.log('startX vs data.x', animatedStartX, data.x)
                }
                return data.x
            })
            .attr('y', function (data) {
                if (data?.data?.name === 'Su Miao') {
                    console.log(data)
                    console.log('startY vs data.y', animatedStartY, data.y)
                    return data.y
                }
                return data.y
            })
            .attr('fillStyle', function (data) {
                if (!data.data.name) {
                    return 'transparent'
                }
                return '#ff0000'
    ...
    drawShowCanvas() {
        ...
        this.virtualContainerNode.selectAll('.orgUnit').each(function () {
            ...
            self.context.fillStyle = node.attr('colorKey')
